...
#talk about running chunks. knit and check the pdf. Do we like this?
...
If you know that most of your chunks need to behave the same qay, you can set global chunk options by running this inside a code chunk. For instance, I usually want the output of my chunks to be part of the document, but I rarely want the actual code to be included in the manuscript. 
...
R Markdown generates a new file that contains selected text, code, and results from the .Rmd file. The new file can be a finished _web page_, _PDF_, _MS Word_ document, _slide show_, _notebook_, _handout_, _book_, _dashboard_, _package vignette_ or many others.
...
1. `New Directory`
1. `New Project`
1. Give a name, pick `Desktop` on your computer hard drive and press `Create`
1. I just sent you a zip file by email now. You can download it here: go.wisc.edu/vj1xh4.
...
a. the chunks that produuce the output you want to cross reference need to be labeled. The label cannot contain underscores, dashes are okay!
...
For this workshop we will work with a .csl I customized. We will also need a .bib file containing the actual bibliographical information we need. This files need to live in the same project file. We specify those two pieces of information in the YAML header like so:
