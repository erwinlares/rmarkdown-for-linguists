...
affiliation: "Department of Spanish a& Portuguese @ UW-Madison"
...
Total number of participants =  ` r native_part + nnative_part`. 
...
Write a descriptive sentence of the data you just read in that indicates the number of observations in the dataset. Hint: the R function you need to include in your code is `nrow()`. This function takes an `R` object as its argument. (2 minutes to work on your own and ask questions)
...
beds %>% 
  select(hardboil, softboil, stove, lid)%>% 
  ...
  geom_boxplot() +
  facet_grid(stove~lid)
```

### Exercise - plots 

Suppose you want to create a plot that shows the distribution of responses for the variable `taste_hb` and you want those responses in a bar plot. Hint: you only need to work with one variable and the geom that you need to use is `geom_bar()`. (5 minutes)
...
\begin{exe} % sets up the top-level example environment (2) 
	...
	\begin{xlist} % subenvironment (2.a)
    	...
    	\ex\label{another-label-c} % subenvironment (2.c)
		\gll estirar la pata \\
		stretch the leg\\
...
My favorite idiom is (\ref{another-label2-c}) because because it is known everywhere in the Spanish-speaking world!
...
\ex\label{tree-03}
  ...
  \begin{forest}
    ...
    [Spec, name=spec][X' 
    [X, name=head][YP]]]
    \draw[->, solid] (spec) to[out=south, in=north west] (head);
  \end{forest}
\end{exe}
```
...
- Don't hesitate to reach out if you have questions, need help or want to collaborate! lares@wisc.edu!
