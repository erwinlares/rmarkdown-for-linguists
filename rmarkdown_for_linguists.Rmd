...
My lofty goal is to show you how to use a tool that will increase the transparency, reproducibility and efficiency of your research. To do this we are going to attempt some _live coding_. I'll start by doing some talking and demoing, then I'll give you time to work on your own document. 

When we are about to start Section 2, I'll share with you a copy of this document you see me working on. As the presentation progresses, we will fill in the blanks together, complete activities, and run the code, so we get an opportunity to practice the strategies and ideas I'm presenting. Don't worry, we'll get errors too!

For now just open RStudio and hand tight.
...
![rmarkdown.document](images/rmarkdown_document.png){width=75%}
...
You can give your document structure by creating sections with headers. A top-level header starts with a single hashtag, second level with 2 hashtags, all the way to 6 levels deep. 
...
For things like glosses and trees we will use a LaTeX environment inside a code chunk.
...
For instance, you might be describing you data and say ... "there are so many observations in the dataset." In the past, you'd have to know the number and type it out. If there were changes to the data, like more people answered your survey, or observations needed to be discarded, you would have to manually update your number. A better option is to ask R to check the value every time you need it. That way if there are changes in the data, those changes will be automatically reflected in your prose. 

You add inline code in-between back ticks, like so: 

Total number of participants =  `native_part + nnative_part`. 

This will create verbatim code. Meaning it would look like code but it won't be actual executable code. To make it actual code, you need to tell RMarkdown what kind language that code is in, so something like:

` r native_part + nnative_part`. 

This will evaluate the expression every time you render your document, so that now the within the scope of the document, anytime there is a change to `native_part` or `nnative_part`, their sum will reflect that change. This is huge for reproducibility!

A code chunk functions similarly. They just take three back ticks and they can contain as many lines of code as you want. You still need to specify the language the code chunk uses inside the curly brackets. The code chunk below loads the libraries we will use throughout the document.
...
If you know that most of your chunks need to behave the same way, you can set global chunk options by running this inside a code chunk. For instance, I usually want the output of my chunks to be part of the document, but I rarely want the actual code to be included in the manuscript. 
...
<!-- Change the global chunk options now. Reknit and check the rendered document -->

## What is happening under the hood

RMarkdown generates a new file that contains selected text, code, and results from the .Rmd file. The new file can be a finished _web page_, _PDF_, _MS Word_ document, _slide show_, _notebook_, _handout_, _book_, _dashboard_, _package vignette_ or many others.
...
1. `New Directory`
1. `New Project`
1. Give a name, I chose `rmarkdown_for_linguists`. Pick `Desktop` on your computer hard drive and press `Create`
...
1. Extract the files and place them in the Desktop/rmarkdown_for_linguist folder that you now have. 
...
We will use a toy data set that I created. I wasn't sure what kind of data I was going to use. I didn't want to play favorites, so today we will work with data related to cooking eggs to perfection because, which as you know together with ramen noodles, eggs are the meal of choice for grad students.
...
Write a descriptive sentence of the data you just read in that indicates the number of observations in the dataset. Hint: the R function you need to include in your code is `nrow()`. (2 minutes)

The dataset BEDS contains `r nrow(beds)`
...
Suppose you want to recreate the equivalent table **but** for the softboiled eggs variable. Go ahead and create a new code chunk and make the necessary changes to create that table. To create a new code chunk you can use `Code -> Insert Chunk` from the `Menu`. (3 minutes)
...
You probably noticed that I inserted an image earlier using `Markdown` syntax. It is likely you also noticed that the result is just a hunk of an image and you had very little control over how it displays. I prefer to use `include_graphics()` from the `knitr` package. Notice all the chunk options.
...
1. you subset by hand the part of the data you needed to create your plot and you moved it to a different sheet in your Excel workbook. Because you are so organized you probably named it something like "section2-plot2" 
...
```{r, fancy-plot, fig.cap="Fancy plot", out.width="4in", fig.align='center', fig.pos="H"}
beds %>% 
  ...
  geom_boxplot() +
  facet_grid(shell~stove)
```

### Exercise - plots 

Suppose you want to create a plot that shows the distribution of responses for the variable `taste_hb` and you want those responses in a barplot. Hint: you only need to work with one variable and the geom that you need to use is `geom_bar()`. (5 minutes)

```{r, fancy-plot-2, fig.cap="Fancy plot", out.width="4in", fig.align='center', fig.pos="H"}

beds %>% 
    select(taste_hb) %>% 
    ggplot(aes(x = taste_hb)) +
    geom_bar()
...
`bookdown::pdf_document2: default`
a. the chunks that produce the output you want to cross reference need to be labeled. The label cannot contain underscores, dashes are okay!
...
Make changes to table and plot you created in the exercises above so you can cross-reference them in a paragraph. (3 minutes)

I created a new table for softboiled eggs, it is table \@ref(tab:fancy-table-2). I also created a plot, plot (\@ref(fig:fancy-plot-2)) shows the results. 
...
For this workshop we will work with a .csl I customized. We will also need a .bib file containing the actual bibliographical information we need. These files need to live in the same project file. We specify those two pieces of information in the YAML header like so:
...
`	\ex\label`{unique-label} <- if you want to cross reference later  
...
My favorite idiom is (\ref{another-label2-c}) because because it is knows everywhere in the Spanish-speaking world!

## Trees

The last thing we will practice is making syntactic trees. Tree drawing also requires a LaTeX environment. You basically need to write your tree in bracket notation. The most basic tree I can think of is something like \@ref(fig:tree-00).  
...
For instance, showing movement with arrows would require you to label the _from_ and _to_ nodes using the `name` option. Then you'll have to actually draw the arrow using the command `\draw`. OJO! `\commands` need to end with a semicolon. They wont `knit` otherwise!
...
That arrow looks wonky, but that is exactly what we asked `Forest` to draw. To tweak the exit and entry points of the arrows, we need to use some basic cartography terminology! Check Figure \@ref(fig:compass) below:
...
\ex\label{tree-03}
  ...
  \begin{forest}
    ...
    [Spec, name=spec][X' 
    [X, name=head][YP]]]
    \draw[->, dotted] (spec) to[out=south, in=north west] (head);
  \end{forest}
\end{exe}
```
...
- If you couldn't finished all the things we did today, you can download a copy of the completed document we worked on today here: https://go.wisc.edu/i1yq5p
- Don't hesitate to reach out if you have questions or need help! lares@wisc.edu!
- Your headings don't work? Make sure there is a space between the hashtag and the text of your heading.
- Did you notice that because the trees we drew have a label, we can cross reference them!
