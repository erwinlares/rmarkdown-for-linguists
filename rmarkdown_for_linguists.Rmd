...
My lofty goal is to show you how to use a tool that will increase the transparency, reproducibility and efficiency of your research. To do this we are going to attempt some _live coding_. You'll have a partial copy of the document and as the presentation progresses, we will together fill in the blanks, complete activities, and run the code, so we get an opportunity to practice the strategies and ideas I'm presenting. 
...
You add inline code in-between back ticks, like so: `x <- a + b`. This will create verbatim code. Meaning it would look like code but it won't be actual executable code. To make it actual code, you need to tell RMarkdown what kind language that code is in, so something like ` r x <- a + b`. This will evaluate the expression of the right and assign it to a variable on the left, so that now the within the scope of the document, anytime there is a change to `a` or `b`, `x` will reflect that change. This is huge for reproducibility!

A code chunk functions similarly. They just take three back ticks and they can contain as many lines of code as you want. You still need to specify the language the code chunk uses(what does that tell you?). The code chunk below loads the libraries we will use throughout the document.
...
You can set different settings for your code chunks. Check an exhaustive list of chunk options by visiting [https://yihui.org/knitr/options](https://yihui.org/knitr/options/)

If you know that most of your chunks need to behave the same, you can set global chunk options by running this inside a code chunk. For instance, I usually want the output of my chunks to be part of the document, but I rarely want the actual code to be included in the manuscript. 
...
1. New Directory
1. New Project
1. Give a name, pick a location on your computer and press Create
1. I just sent you a zip file by email now. You can download it here: go.wisc.edu/
1. Place that file in the Documents/Rmarkdown_for_linguist folder that you now have.
1. Go back to RStudio. Check the Files tab, and click on the rmarkdown_for_linguists.Rmd 
...
We will use a toy data set that I created. I wasn't sure what kind of data I was going to use. I didn't want to play favorites, so today we will work with data related to boiling eggs to perfection.
...
Write a descriptive sentence of the data you just read in that indicates the number of observations and the numbers of variables in the dataset. Hint: the R functions you need are `nrow()` and `colnames()`. (2 minutes)

There are `r nrow(beds)` observations in the dataset. The variables recorded are: `r colnames(beds)`.
...
For our analysis we want a table that shows the mean cooking time for hard boiled eggs across the type of shell and the type of stove used.
...
Supose you want to recreate the equivalent table for the softboiled eggs. Go ahead and create a new code chunk and make the necessary changes to create that table. To create a new code chunk you can use `Code -> Insert Chunk` from the `Menu`. (3 minutes)
...
beds %>% select(shell, stove, softboil) %>% 
    group_by(shell, stove) %>% 
    summarize(mean_sb = mean(softboil)) %>% 
#kable()  
  kable(format = "latex",
        booktabs = TRUE,
        digits = 2, 
...
You probably noticed that I inserted an image earlier using `Markdown` syntax. It is likely also noticed that the result is just a hunk of an image and you had very little control over how it displays. I prefer to use `include_graphics()` from the `knitr` package.
...
Suppose you want to create a plot that shows the distribution of responses for the variable `taste_hb` and you want those responses in a barplot. (5 minutes)

```{r, fancy-plot2, fig.cap="Fancy plot 2", out.width="4in", fig.align='center'}
beds %>% 
...
a. a special kind of output from the `bookdown` package needs to be specified in the YAML header. Careful with the indentation!
    `bookdown::pdf_document2: default`
a. the chunks containing the code need to be labeled. The label cannot contain underscores, dashes are okay!
...
Make changes to your table and plot so you can cross-reference them. (3 minutes)

Plot \@ref(fig:fancy-plot2) shows that the most common answer is 4.  
Table \@ref(tab:fancy-table2) shows that for brown-shelled eggs, the type of stove made a difference when cooking hardboiled eggs. No difference was observed for softboiled eggs. For white-shelled eggs, the type of stove made no difference between the cooking time for soft or hard boiled eggs.  
...
If you have examples that require glosses or multiple instances of one example, you'll need to create a subenvironment with `{xlist}` within the parent `{exe}` environment, like so:
...
Add your favorite idiom as a third example and cross-reference below indicating why. 

## Trees

The last thing we will practice is making syntactic trees. Tree drawing also requires a LaTeX environment and you basically write your tree in bracket notation. The most basic tree I can think of is something like \@ref(fig:tree-00).  
...
And there us our tree! All the things you can with LaTeX, you can do here since it is a TeX environment! If you want to take a deep dive, check the [Forest package](https://texdoc.org/serve/forest/0)
...
- RMarkdown lets you create more than just pdfs: my personal website is done with `blogdown`. I make my  slides`xaringan`. Check [erwinlares.com](erwinlares.com)!
- You can download a copy of the completed document we worked on today here: https://go.wisc.edu/i1yq5p
